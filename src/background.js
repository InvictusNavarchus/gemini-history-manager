...
function updateBadge(count) {
  Logger.debug(`Updating badge with count: ${count}`);
  
  // Update local state
  const previousCount = STATE.historyCount;
  STATE.historyCount = count || STATE.historyCount;
  
  Logger.debug(`Badge count: previous=${previousCount}, new=${STATE.historyCount}`);
  ...
  Logger.log(`Badge updated: ${STATE.historyCount > 0 ? STATE.historyCount.toString() : 'empty'}`);
}
...
function initializeExtension() {
  Logger.log('Background script initializing');
  
  // Get history count from storage and update badge
  Logger.debug('Loading history data from storage to update badge');
  browser.storage.local.get('geminiChatHistory')
    .then(data => {
      const history = data.geminiChatHistory || [];
      Logger.debug(`Loaded ${history.length} history items from storage`);
      updateBadge(history.length);
      Logger.log(`Badge updated with count: ${history.length}`);
    })
    .catch(error => {
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  const senderInfo = sender.tab ? 
    ...
    'from extension page';
  Logger.debug(`Message source: ${senderInfo}`);
  
  switch (message.action) {
    case 'updateHistoryCount':
      Logger.log(`Updating badge count to ${message.count}`);
      updateBadge(message.count);
      break;
      
    case 'openHistoryPage':
      Logger.log('Opening dashboard/history page in new tab');
      browser.tabs.create({
        url: '/dashboard/dashboard.html'
      }).then(tab => {
        Logger.debug(`Dashboard opened in tab ID: ${tab.id}`);
      }).catch(err => {
        Logger.error('Error opening dashboard tab:', err);
      });
      break;
      
    case 'getHistoryCount':
      Logger.log(`Responding with current history count: ${STATE.historyCount}`);
      sendResponse({ count: STATE.historyCount });
      break;
      
    default:
      Logger.warn(`Received unknown action: ${message.action}`);
  }
});
...
browser.runtime.onInstalled.addListener((details) => {
  Logger.log(`Extension ${details.reason} event triggered`);
  Logger.debug(`Install details: ${JSON.stringify({
  ...
  if (details.reason === 'install') {
    ...
    Logger.log('Extension installed for the first time - initializing setup');
...
browser.runtime.onStartup.addListener(() => {
  Logger.log('Browser started - initializing extension');
  Logger.debug('onStartup event triggered - this happens when browser starts with extension already installed');
});
...
Logger.log('Background script loaded - starting initialization');
initializeExtension();