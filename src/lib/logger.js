...
/**
 * Creates a content-script compatible version of the logger
 * @param {Function} isLoggingEnabledFn - The function to check if logging is enabled
 * @returns {Object} A logger object that can be used in content scripts
 */
export function createContentScriptLogger(isLoggingEnabledFn) {
  return {
    LOG_PREFIX: "[Gemini History]",
    ...
    _log: function (method, context, message, error, ...args) {
      const logLevel = method.toLowerCase();
      if (!isLoggingEnabledFn(context, logLevel)) {
        return;
      }
