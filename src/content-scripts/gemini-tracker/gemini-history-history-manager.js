(function () {
  ...
  const HistoryManager = {
    ...
    loadHistory: function () {
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Loading history from storage...`);
        ...
        browser.storage.local
          ...
          .then((data) => {
            ...
            if (Array.isArray(history)) {
              console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] History loaded successfully. Found ${history.length} entries.`);
              resolve(history);
            } else {
              console.warn(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Stored history data is not an array. Returning empty history.`);
              resolve([]);
            }
          })
          .catch((error) => {
            console.error(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Error loading history:`, error);
    ...
    saveHistory: function (history) {
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Attempting to save history with ${history.length} entries...`);
      if (!Array.isArray(history)) {
        console.error(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Attempted to save non-array data. Aborting save.`);
      ...
      return browser.storage.local
        ...
        .then(() => {
          console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] History saved successfully.`);
          ...
          browser.runtime
            ...
            .catch((err) => console.error(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Error sending message to background:`, err));
        })
        .catch((error) => {
          console.error(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Error saving history:`, error);
    ...
    addHistoryEntry: async function (
      ...
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Attempting to add history entry:`, entryData);
      ...
      try {
        ...
        if (history.some((entry) => entry.url === url)) {
          console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Duplicate URL detected, skipping entry:`, url);
        ...
        console.log(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Successfully added history entry.`);
        StatusIndicator.show(`Chat "${title}" saved to history`, "success");
        return true; // Indicate success
      } catch (error) {
        console.error(`[${new Date().toTimeString().slice(0, 8)}] [gemini-tracker] Error adding history entry:`, error);
