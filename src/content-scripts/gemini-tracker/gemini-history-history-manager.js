(function () {
  ...
  const HistoryManager = {
    ...
    loadHistory: function () {
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Loading history from storage...`);
        ...
        browser.storage.local
          ...
          .then((data) => {
            ...
            if (Array.isArray(history)) {
              console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] History loaded successfully. Found ${history.length} entries.`);
              resolve(history);
            } else {
              console.warn(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Stored history data is not an array. Returning empty history.`);
              resolve([]);
            }
          })
          .catch((error) => {
            console.error(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Error loading history:`, error);
    ...
    saveHistory: function (history) {
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Attempting to save history with ${history.length} entries...`);
      if (!Array.isArray(history)) {
        console.error(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Attempted to save non-array data. Aborting save.`);
      ...
      return browser.storage.local
        ...
        .then(() => {
          console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] History saved successfully.`);
          ...
          browser.runtime
            ...
            .catch((err) => console.error(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Error sending message to background:`, err));
        })
        .catch((error) => {
          console.error(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Error saving history:`, error);
    ...
    addHistoryEntry: async function (
      ...
      console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Attempting to add history entry:`, entryData);

      // Basic validation (Title, URL, Timestamp, Model are still required)
      if (!timestamp || !url || !title || !model) {
        console.warn(
          `[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Attempted to add entry with missing essential data. Skipping.`,
          entryData
        );
      ...
      if (!Utils.isValidChatUrl(url)) {
        console.warn(
          `[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Attempted to add entry with invalid chat URL pattern "${url}". Skipping.`
        );
      ...
      try {
        ...
        if (history.some((entry) => entry.url === url)) {
          console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Duplicate URL detected, skipping entry:`, url);
        ...
        console.log(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Successfully added history entry.`);
        StatusIndicator.show(`Chat "${title}" saved to history`, "success");
        return true; // Indicate success
      } catch (error) {
        console.error(`[${new Date().toTimeString().slice(0, 8)}] [GHM] [gemini-tracker] Error adding history entry:`, error);
