...
export function createToastManager() {
  ...
  function showToast(message, type = 'info', duration = 5000) {
    Logger.log("uiHelpers", `🍞 Toast #${state.toastIdCounter}: Creating new toast with message: "${message}", type: ${type}, duration: ${duration}ms`);
    ...
    Logger.log("uiHelpers", `🍞 Toast #${id}: Current active toasts count before adding: ${state.activeToasts.length}`);
    
    state.activeToasts.push(newToast);
    
    Logger.log("uiHelpers", `🍞 Toast #${id}: Toast added to activeToasts array. New length: ${state.activeToasts.length}`);
    Logger.log("uiHelpers", `🍞 Toast #${id}: Active toasts IDs: ${state.activeToasts.map(t => t.id).join(', ')}`);
  ...
  function removeToast(id) {
    ...
    Logger.log("uiHelpers", `🍞 Toast #${id}: Current active toasts before removal: ${state.activeToasts.length}`);
    ...
    if (index !== -1) {
      ...
      Logger.log("uiHelpers", `🍞 Toast #${id}: Toast removed successfully. New active toasts count: ${state.activeToasts.length}`);
    } else {
  ...
  function getActiveToasts() {
    ...
    if (state.activeToasts.length > 0) {
      Logger.log("uiHelpers", `🍞 Toast Manager: Active toast IDs: ${state.activeToasts.map(t => t.id).join(', ')}`);
    }
...
export function downloadFile(data, filename, type = 'application/json') {
  ...
  try {
    ...
    try {
      ...
      Logger.debug("uiHelpers", `Successfully stringified ${Array.isArray(data) ? data.length + ' items' : 'object'} for download`);
    } catch (jsonError) {
...
export function exportHistoryData(dataToExport, isFiltered = false) {
  ...
  try {
    ...
    if (dataToExport.length > 0) {
      ...
      Logger.debug("uiHelpers", `Export sample item - ID: ${firstItem.id}, Title: ${firstItem.title}, Date: ${firstItem.timestamp}`);
    }
